...
جدول‌های `bot_users`، `telegram_updates`، `consultation_requests` و `contacts` در اولین درخواست به‌صورت خودکار ساخته می‌شوند. شماره‌های موجود قبلی نیز هنگام ایجاد جدول مخاطبین به آن منتقل می‌شوند. فایل‌های migration در پوشه `migrations` نگهداری می‌شوند.
